...
static void redraw_badge(DefaultHandles *p) {
  ...
  if (not_my_badge) {
    ...
    sprintf(tmp, "%s", "Allied Mastercomputer");
  }
  ...
  // level
  if (not_my_badge) {
    sprintf(tmp, "%s", "LEVEL 0");
  } else {
    sprintf(tmp, "%s", "LEVEL 13");
  }
  gdispDrawStringBox (0,
          ypos,
          gdispGetWidth(),
  ...
  if (not_my_badge) {
    sprintf(tmp, "%s", "game,rank-0.rgb");
  } else {
    sprintf(tmp, "%s", "game/rank-12.rgb");
  }
...
static uint32_t ddefault_init(OrchardAppContext *context) {
  (void)context;
...
static void ddefault_start(OrchardAppContext *context) {
  DefaultHandles * p;
...
static void ddefault_event(OrchardAppContext *context,
...
static void ddefault_exit(OrchardAppContext *context) {
  DefaultHandles * p;
...
orchard_app("Bbadge", "icons/bbadge.rgb", 0, ddefault_init, ddefault_start,
	ddefault_event, ddefault_exit, 0);
